...
# --- File Paths ---
csv_files = {
    "transactions": "C:/Users/Akshiya George/OneDrive/Desktop/Data Science/Amazon/transactions.csv",
    "products": "C:/Users/Akshiya George/OneDrive/Desktop/Data Science/Amazon/products.csv",
    "customers": "C:/Users/Akshiya George/OneDrive/Desktop/Data Science/Amazon/customers.csv",
    "time_dimension": "C:/Users/Akshiya George/OneDrive/Desktop/Data Science/Amazon/time_dimension.csv"
}
...
@st.cache_data
def load_data():
    tx = pd.read_csv(csv_files["transactions"])
    pr = pd.read_csv(csv_files["products"])
    cu = pd.read_csv(csv_files["customers"])
...
df, products_df, customers_df = load_data()
...
# ✅ Single dropdown
selected_label = st.sidebar.selectbox("📂 Choose a Dashboard", list(options.keys()))
selected_table, view_type = options[selected_label]
df = load_csv(csv_files[selected_table])
...
if view_type == "executive":
    ...
    df = load_csv(csv_files["transactions"])
    prod_df = load_csv(csv_files["products"])
    ...
    # ✅ Define targets (can be dynamic or pulled from config)
    ...
    transactions_df = load_csv(csv_files["transactions"])
    time_df = load_csv(csv_files["time_dimension"])

    # ✅ Ensure 'order_date' exists and is datetime in both DataFrames
    ...
    # ✅ Load product data and merge category info
    prod_df = load_csv(csv_files["products"])
    df = df.merge(prod_df[["product_id", "category"]], on="product_id", how="left")
    ...
    # Load transactions data specifically for growth analysis
    df = load_csv(csv_files["transactions"])
    ...
    # 🌤️ Seasonal Variation (Monthly only)
    if period == "Monthly":
    ...
    # ✅ Merge customer location info (excluding lat/lon)
    ...
    if "discount_percent" in transactions_df.columns:
        discount_df = transactions_df.groupby("discount_percent")["final_amount_inr"].sum().sort_values(ascending=False)
        st.subheader("🎯 Revenue by Discount Percent")
    ...
    # Load transactions and merge with customers
    tx_df = load_csv(csv_files["transactions"])
    ...
    if "customer_id" in tx_df.columns:
        ...
        cust_df = load_csv(csv_files["customers"])
    ...
    if "customer_age_group" in df.columns and "final_amount_inr" in df.columns:
        ...
        spend_by_age = df.groupby("customer_age_group")["final_amount_inr"].mean()
    ...
    # ✅ Load product and transaction data
    prod_df = load_csv(csv_files["products"])
    tx_df = load_csv(csv_files["transactions"])
    ...
    if "order_month" in tx_df.columns and "order_year" in tx_df.columns:
    ...
    # ✅ Validate required CSV keys
    required_keys = ["customers", "products", "transactions", "time_dimension"]
    missing_keys = [key for key in required_keys if key not in csv_files]
    if missing_keys:
        st.error(f"Missing required CSV files: {', '.join(missing_keys)}")
        st.stop()
    ...
    # ✅ Merge product details (rating, brand, etc.)
    ...
    # ✅ Merge customer location info into transactions
    ...
    if missing_cols:
        ...
        # ⚙️ Operational Efficiency: Delivery Type Breakdown
        ...
        # 🚩 Delay Flags: States with Avg Delivery > 7 Days
    ...
    # 🔍 Drill-down: Payment Method Selection
    ...
    # ✅ Merge product category info into transactions
    tx_df = tx_df.merge(products_df[["product_id", "category"]], on="product_id", how="left")
    ...
    # ✅ Load transactions, customers, and products data
    tx_df = load_csv(csv_files["transactions"])
    cust_df = load_csv(csv_files["customers"])
    prod_df = load_csv(csv_files["products"])
    ...
    # 🔮 Placeholder: Forecasting logic (e.g., ARIMA, Prophet)
    ...
    # ✅ Merge pricing from transactions into product data
    ...
    # ✅ Check for rating and category columns
    ...
    # ✅ Check for pricing columns after merge
    ...
    # ✅ Recommendation Effectiveness (Repeat purchase rate)
    ...
    if missing_cols:
        ...
        total_orders = len(tx_df)  # fallback if 'order_id' is missing
